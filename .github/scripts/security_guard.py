...
def identity_match(values: list[str]) -> bool:
    candidates = list(values)
    candidates += ["".join(values[i:i + 2]) for i in range(max(0, len(values) - 1))]
    candidates += ["".join(values[i:i + 3]) for i in range(max(0, len(values) - 2))]
...
def main() -> int:
    ...
    for path in tracked_paths():
        ...
        if name.startswith(".env") and name not in ALLOWED_ENV:
            findings.add(f"{path}: tracked environment file")
        if name in FORBIDDEN_NAMES or (name.startswith("credentials.") and name.endswith(".json")):
            findings.add(f"{path}: tracked credential file")
        if path.suffix.lower() in FORBIDDEN_SUFFIXES:
            findings.add(f"{path}: tracked key or credential container")
        ...
        if b"\0" in data:
            ...
            if identity_match(binary_tokens):
                findings.add(f"{path}: forbidden personal identifier in binary data")
            continue
        for line_number, line in enumerate(data.decode("utf-8", "replace").splitlines(), 1):
            if identity_match(tokens(line)):
                findings.add(f"{path}:{line_number}: forbidden personal identifier")
