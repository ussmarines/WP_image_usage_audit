...
def main():
 ...
 g+=['--redact=100','--exit-code=2','--report-format=json',f"--report-path={reports/'gitleaks.json'}"];run('gitleaks',g,r,repo);run('opengrep',[t['opengrep']['executable'],'scan','--config',str(repo/'.security/opengrep/project-security.yml'),'--json-output',str(reports/'opengrep.json'),'--error','--exclude','node_modules','--exclude','vendor','--exclude','dist',str(repo)],r,repo);tv=t['trivy']['executable'];run('trivy',[tv,'filesystem','--scanners','vuln,misconfig','--severity','MEDIUM,HIGH,CRITICAL','--format','json','--output',str(reports/'trivy.json'),'--exit-code','1',str(repo)],r,repo);run('sbom',[tv,'filesystem','--scanners','vuln','--format','cyclonedx','--output',str(reports/'sbom.cdx.json'),str(repo)],r,repo)
